...
JSONL dataset input
SQL Layer (SQLite) + Graph Layer (NetworkX)
Query Planner (LLM-assisted intent classification and structured plan)
Execution Engine (SQL for analytics, graph for traces)
FastAPI backend
Optional UI layer (graph view + chat interface)
...
Hybrid approach
We didn’t pick “graph only” or “SQL only.” We use the tool that fits each need:
